...
def connect_exchange():
    ...
    if TESTNET:
        ...
        logger.info("🧪 Connected to BINANCE TESTNET (no real money)")
    else:
...
def execute_trade(exchange, signal, candle, risk_mgr):
    ...
    if signal == "BUY":
        ...
        if base_qty < 0.0001:
            logger.info("ℹ️  SELL signal but no BTC position held, skipping")
            return
...
def send_periodic_report(exchange):
    ...
    try:
        ...
        logger.info("📩 4-Hour report sent via email")
